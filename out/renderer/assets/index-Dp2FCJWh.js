...
let blueSphere = {
  ...
  radius: 1,
  color: { r: 0, g: 0, b: 255 },
  // Updated for Tweakpane color compatibility
  metalness: 0,
  roughness: 0.5,
  emissive: { r: 0, g: 0, b: 0 },
  // Same here for emissive
  anisotropy: 0,
  subsurface: 0,
...
let keysPressed = /* @__PURE__ */ new Set();
...
document.addEventListener("mousemove", (event) => {
  if (cameraControlActive && isDragging) {
    ...
    const rotationSensitivity = 2e-3;
    camera.rot[1] += deltaX * rotationSensitivity;
    camera.rot[0] += deltaY * rotationSensitivity;
...
function moveCamera() {
  ...
  const right = [Math.sin(camera.rot[1] - Math.PI / 2), 0, Math.cos(camera.rot[1] - Math.PI / 2)];
  const up = [0, 1, 0];
